                                 ...
                                 : _server(serverPort), _lwipVersion{0, 0, 0}
{
  ...
  updateNetworkNames(meshName, (nodeID != "" ? nodeID : uint64ToString(ESP.getEfuseMac())));
...
void ESP8266WiFiMesh::attemptTransmission(const String &message, bool concludingDisconnect, bool initialDisconnect, bool noScan, bool scanAllWiFiChannels)
{
  ...
  else
  {
    if(!noScan)
    {
      ...
      {
        // Scan function argument overview: scanNetworks(bool async = false, bool show_hidden = false, uint8_t channel = 0, uint8* ssid = NULL)
      ...
      if(_verboseMode) // Avoid string generation if not required
      {
        ...
        if(currentNetwork.networkIndex != NETWORK_INFO_DEFAULT_INT)
        {
          verboseModePrint("(" + String(WiFi.RSSI(currentNetwork.networkIndex)) + String(F("dBm) ")) + 
                           (WiFi.encryptionType(currentNetwork.networkIndex) == WIFI_MODE_NULL ? String(F("open")) : ""), false);
        }
